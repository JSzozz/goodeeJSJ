...
--%>
	<script>
		$("#memberAll").click(e=>{
			$.get("<%=request.getContextPath()%>/memberList.do", /* com.web.member.controller - AjaxUserListServlet */
					data=>{
						$("#memberList").html("");
						const table=$("<table>");
						const header=$("<tr>");
		...
		$("#memberSearchById").click(e=>{
			if($('#Id').val()!=""){
			$.ajax({
				url:"<%=request.getContextPath()%>/membersSearchById.do", /* com.web.member.controller - AjaxMemberListById */
				dataType:"json", 
				data:{id:$('#Id').val()}, 
				success:(data)=>{
					console.log(data);
