...
public class QnaSearchFilterServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String qnaFilter=request.getParameter("QnaFilter");
		System.out.println("flag : "+qnaFilter);
		String filter = "";
		if(qnaFilter.isEmpty()) {
			
		}else {
			filter = qnaFilter.substring(0, qnaFilter.length()-1); //filter 									
		}
...
//		Map map=Map.of("type",searchType,"keyword",keyword);
		...
		List<Qna> Qnas=new QnaService().searchQnaFilter(filter,pagemap); //게시물 가져오기
		//2. DB에서 가져온 데이터 저장(화면출력)
		System.out.println(Qnas);
		request.setAttribute("Qnas", Qnas);
		//3. 페이지바를 구성
		// 1) DB에 저장된 테이블에서 조건에 해당하는 데이터의 수를 가져오기
		int totalData=new QnaService().selectQnaFilterCount(filter);
