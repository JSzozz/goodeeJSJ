...
public class BoardCommentInsertServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String view;
		if(result>0) {
			/* view="/views/board/boardView.do"; */
			view=request.getContextPath()+"/board/boardView.do?no="+bc.getBoardRef();//V : nullpointException예방
			response.sendRedirect(view);//sendRedircet는 insertComment를 방지함
		}else {
