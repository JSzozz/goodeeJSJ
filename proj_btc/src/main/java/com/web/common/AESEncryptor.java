...
//			//key를 저장할 파일명은 bslove.bs;// 파일명 앞에 .찍으면 숨김파일 처리가능!
...
//			System.out.println(path+"(*AESEncryptor_AESEncryptor()실행)");
//	///C:/Users/GDC10/git/repository/06_HelloMVC/src/main/webapp/WEB-INF/(*MyContextListener_AESEncryptor()실행
//			
...
//		   //암호화처리하는 메소드
//		   Cipher cipher = Cipher.getInstance("AES");//import
//		   cipher.init(Cipher.ENCRYPT_MODE, AESEncryptor.key);
////		   ENCRYPT_MODE : int 값, static변수
//		   byte[] targetData=oriData.getBytes(Charset.forName("UTF-8"));//byte화
//		   byte[] encryptData=cipher.doFinal(targetData);//암호화
//		   
...
//		   
//		   byte[] encrypt= Base64.getDecoder().decode(encryptData.getBytes(Charset.forName("UTF-8")));//복호화
//		   byte[] decryptData=cipher.doFinal(encrypt);//복호화
//		   
