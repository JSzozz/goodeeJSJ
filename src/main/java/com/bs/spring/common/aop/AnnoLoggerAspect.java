...
public class AnnoLoggerAspect {
	...
	@After("memoLogger()")
	public void afterLogger(JoinPoint jp){//Joinpoint (x)
		log.debug("============== annotation aop after start========");
		Signature sig=jp.getSignature();
		log.debug(sig.getDeclaringTypeName()+" "+sig.getName());
		log.debug("============== annotation aop after end========");
	}
	...
	public Object daoLogger(ProceedingJoinPoint join) throws Throwable{
		...
		stop.start();
		log.debug("=========== around logger dao before start===============");
		log.debug("------- 전처리 내용 구현 --------------");
		log.debug("=========== around logger dao before end===============");
		Object obj=join.proceed();
		stop.stop();
		log.debug("=========== around logger dao  after start===============");
		log.debug("------- 후처리 내용 구현 --------------");
